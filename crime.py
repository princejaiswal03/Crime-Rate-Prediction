...
app.config['MAIL_USERNAME']='prince.crimerate@gmail.com'
app.config['MAIL_PASSWORD']='$0pi@123'
...
def dashboard_yearwise_graph_analysis():
    ...
    if request.method == 'POST':
        ...
        #calculating the max index
        for i,j in previous_pair.iteritems():
...
def dashboard_crimewise_graph_analysis():
    ...
    if request.method == 'POST':
        ...
        for i,j in after_pair.iteritems():
            after_total_crime += j
    ...
    return render_template('dashboard_crimewise_graph_analysis.html',crime_data=data,crime_name=crime_name,total_crime=total_crime,pair=pair.iteritems(),max_value=max_value,after_total_crime=after_total_crime,after_pair=after_pair.iteritems(),after_max_value=after_max_value)
...
def dashboard_variation_graph():
    ...
    if request.method == 'POST':
        ...
        if request.form['submit'] == "See Graph Crime Type And Number of Times":
            ...
            #flash(first_crime_count)
            first_crime_count=first_crime_count.iteritems()
            second_crime_count= dict(zip(crime_obj.crime_type_data,second_crime_count))
            #flash(second_crime_count)
            second_crime_count=second_crime_count.iteritems()
